...
namespace alpr
{
  ...
  AlprResults Alpr::recognize(unsigned char* pixelData, int bytesPerPixel, int imgWidth, int imgHeight, cv::Mat frm_ocr, std::vector<AlprRegionOfInterest> regionsOfInterest)
  {
    return impl->recognize(pixelData, bytesPerPixel, imgWidth, imgHeight, frm_ocr, regionsOfInterest);
  }

  std::queue<PlateRegion> Alpr::recognizeregion(unsigned char* pixelData, int bytesPerPixel, int imgWidth, int imgHeight, std::vector<AlprRegionOfInterest> regionsOfInterest)
  {
    return impl->recognizeregion(pixelData, bytesPerPixel, imgWidth, imgHeight, regionsOfInterest);
  }

  AlprResults Alpr::recognizeresult(cv::Mat colorImg, cv::Mat grayImg, std::queue<PlateRegion> plateQueue)
  {
    return impl->recognizeresult(colorImg, grayImg, plateQueue);
  }

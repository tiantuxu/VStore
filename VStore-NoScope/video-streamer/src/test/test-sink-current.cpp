...
bool flag_720[50];                           //flag indicate that it is currently feeding 720p video chunk to the client
bool flag_540[50];
int chunk_num=5;
/*
...
//	receiver.connect(CLIENT_PULL_FROM_ADDR);
    ...
    while(1){
        ...
        #pragma omp parallel for schedule(dynamic, 1250) num_threads(30)
        for (int j = rd.start_fnum; j < rd.start_fnum + rd.total_fnum; j++) {
            ...
            if (regionsOfInterest.size() > 0){
                ...
                rd_one_frame.db_seq = SSD_RAW_720;
                ...
                //Specify the frame wants to retrieve
                rd_one_frame_ocr.db_seq = SSD_CHUNK_540;
                ...
                if(j-status_720[id] > chunk_num*CHUNK_SIZE){
                    for(int i=0;i<chunk_num;i++)
                    {
                        ...
                        for (int iter = 0; iter < rd_one_frame.total_fnum; iter++) {
                            ...
                            pre_buf_720[id][iter+CHUNK_SIZE*i] = msg_ptr;
                        }
                        rd_one_frame.db_seq = SSD_RAW_720;
                ...
                if(j-status_720[id]<=CHUNK_SIZE)
                    flag_720[id]=false;                                   //terminate streaming

                /********** Requesting 720 New Frames **********/
                if(j - status_720[id] == 4*CHUNK_SIZE||flag_720[id]==true) {
                    flag_720[id]=true;
                    rd_one_frame.db_seq = SSD_RAW_720;
                ...
                // The next snippet works for the raw video retrieval
#if 0
                    /********** Initializing 5 CHUNKS of New Frames (540p)  **********/
                    ...
                    if(j-status_540[id] > chunk_num*CHUNK_SIZE){
                        for(int i=0;i<chunk_num;i++)
                        {
                            ...
                            for (int iter = 0; iter < rd_one_frame_ocr.total_fnum; iter++) {
                                msg_ptr = recv_one_chunk_720(*worker_socket[id]);
                                pre_buf_540[id][iter+CHUNK_SIZE*i] = msg_ptr;
                            }
                            rd_one_frame_ocr.db_seq = SSD_CHUNK_540;
                    ...
                    /********** Stop Requesting 540 New Frames **********/
                    if(j-status_540[id]<=CHUNK_SIZE)
                        flag_540[id]=false;

                    /********** Requesting 540 New Frames **********/
                    if(j - status_540[id] == 4*CHUNK_SIZE||flag_540[id]==true) {
                        flag_540[id]=true;
                        rd_one_frame_ocr.db_seq = SSD_CHUNK_540;

                        int N=chunk_num*CHUNK_SIZE-(j-status_540[id]);
                    ...
                    msg_ptr = pre_buf_540[id][j%(chunk_num*CHUNK_SIZE)];
#endif
...
#if 1
                    /********** Prefetching one chunk of  540 New Frames after the computation **********/
                    if(j - status_540[id] == 4*CHUNK_SIZE||flag_540[id]==true) {

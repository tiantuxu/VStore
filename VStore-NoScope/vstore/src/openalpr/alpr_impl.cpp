...
*/
  ...
  /* teddyxu: Same recognizeFullDetails with imgOcr matrix */
  AlprFullDetails AlprImpl::recognizeFullDetails(cv::Mat img, cv::Mat imgOcr, std::vector<cv::Rect> regionsOfInterest)
  {
    timespec startTime;
    getTimeMonotonic(&startTime);
...
*/
    ...
    Mat grayImgOcr = imgOcr;
    if (imgOcr.channels() > 2)
      cvtColor( imgOcr, grayImgOcr, CV_BGR2GRAY );
    ...
    grayImgOcr = prewarp->warpImage(grayImgOcr);
    // Iterate through each country provided (typically just one)
    // and aggregate the results if necessary
    ResultAggregator country_aggregator(MERGE_PICK_BEST, topN, config);
    for (unsigned int i = 0; i < config->loaded_countries.size(); i++)
    {
      ...
      for (unsigned int iteration = 0; iteration < config->analysis_count; iteration++)
      {
        Mat iteration_image = iter_aggregator.applyImperceptibleChange(grayImg, iteration);
        //drawAndWait(iteration_image);
        AlprFullDetails iter_results = analyzeSingleCountry(img, iteration_image, imgOcr, grayImgOcr, warpedRegionsOfInterest);
        iter_aggregator.addResults(iter_results);
      }
    ...
    /* teddyxu: Same recognizeFullDetailsRegions */
    std::queue<PlateRegion> AlprImpl::recognizeFullDetailsRegion(cv::Mat img, std::vector<cv::Rect> regionsOfInterest)
    {
        timespec startTime;
        getTimeMonotonic(&startTime);
...
*/
        ...
        for (unsigned int i = 0; i < config->loaded_countries.size(); i++)
        {
            ...
            for (unsigned int iteration = 0; iteration < config->analysis_count; iteration++)
            {
                Mat iteration_image = iter_aggregator.applyImperceptibleChange(grayImg, iteration);
                //drawAndWait(iteration_image);
                plateQueue = analyzeSingleCountryRegion(img, iteration_image, warpedRegionsOfInterest);
                //iter_aggregator.addResults(iter_results);
            }
  ...
  AlprFullDetails AlprImpl::analyzeSingleCountry(cv::Mat colorImg, cv::Mat grayImg, std::vector<cv::Rect> warpedRegionsOfInterest)
  {
    ...
    AlprFullDetails AlprImpl::analyzeSingleCountryResult(cv::Mat colorImg, cv::Mat grayImg, std::queue<PlateRegion> plateQueue)
    {
        AlprFullDetails response;
        //cout << "analyzeSingleCountryResult" << endl;
        ...
        while(!plateQueue.empty())
        {
            ...
            //cout << plateRegion.rect.x << endl;
            //cout << plateRegion.rect.y << endl;
            //cout << plateRegion.rect.width << endl;
            //cout << plateRegion.rect.height << endl;

            plateRegion.rect.x = int(3 * plateRegion.rect.x/4);
            plateRegion.rect.y = int(3 * plateRegion.rect.y/4);
            plateRegion.rect.width = int(3 * plateRegion.rect.width/4);
            plateRegion.rect.height = int(3 * plateRegion.rect.height/4);
    ...
    /* teddyxu: Same analyzeSingleCountry returns a plate region queue */
    std::queue<PlateRegion> AlprImpl::analyzeSingleCountryRegion(cv::Mat colorImg, cv::Mat grayImg, std::vector<cv::Rect> warpedRegionsOfInterest)
    {
        AlprFullDetails response;
        //cout << "analyzeSingleCountry" << endl;
        ...
        vector<PlateRegion> warpedPlateRegions;
        //float ratio = colorImgOcr.rows/colorImg.rows;
        //cout << "colorImgOcr.rows = " << colorImgOcr.rows << endl;
        float ratio = 0.75;
...
/*
        for(unsigned int i = 0; i < warpedPlateRegions.size(); i++){
            cout << warpedPlateRegions[i].rect.x << " " << warpedPlateRegions[i].rect.y << " " << warpedPlateRegions[i].rect.width << " " << warpedPlateRegions[i].rect.height << " " << endl;
            warpedPlateRegions[i].rect.x = int(ratio * warpedPlateRegions[i].rect.x);
            warpedPlateRegions[i].rect.y = int(ratio * warpedPlateRegions[i].rect.y);
            warpedPlateRegions[i].rect.width = int(ratio * warpedPlateRegions[i].rect.width);
            warpedPlateRegions[i].rect.height = int(ratio * warpedPlateRegions[i].rect.height);
            cout << warpedPlateRegions[i].rect.x << " " << warpedPlateRegions[i].rect.y << " " << warpedPlateRegions[i].rect.width << " " << warpedPlateRegions[i].rect.height << " " << endl;
        }
*/
        queue<PlateRegion> plateQueue;
        ...
        //cout << "platequeue.size() = " << plateQueue.size() << endl;

        return plateQueue;
    }

  /* teddyxu: Same analyzeSingleCountry with ocr frame */
  //AlprFullDetails AlprImpl::analyzeSingleCountry(cv::Mat colorImg, cv::Mat grayImg, cv::Mat colorImgOcr, cv::Mat grayImgOcr, std::vector<cv::Rect> warpedRegionsOfInterest)
  AlprFullDetails AlprImpl::analyzeSingleCountry(cv::Mat colorImg, cv::Mat grayImg, cv::Mat colorImgOcr, cv::Mat grayImgOcr, std::vector<cv::Rect> warpedRegionsOfInterest)
  {
    AlprFullDetails response;
    //cout << "analyzeSingleCountry" << endl;
    ...
    while(!plateQueue.empty())
    {
      ...
      PipelineData pipeline_data(colorImgOcr, grayImgOcr, plateRegion.rect, config);
      //cout << plateRegion.rect.x << " " << plateRegion.rect.y << " " << plateRegion.rect.width << " " << plateRegion.rect.height << " " << endl;
      pipeline_data.prewarp = prewarp;
	...
	AlprPlateResult plateResult;
        ...
        cout << "pipeline_data_rect_size = "<< pipeline_data.regionOfInterest.height << " " <<pipeline_data.regionOfInterest.width << endl;
        country_recognizers.ocr->performOCR(&pipeline_data);
        country_recognizers.ocr->postProcessor.analyze(plateResult.region, topN);
  ...
  AlprResults AlprImpl::recognize( unsigned char* pixelData, int bytesPerPixel, int imgWidth, int imgHeight, std::vector<AlprRegionOfInterest> regionsOfInterest)
  {
    try
    {
	  ...
	  //getTimeMonotonic(&endTime);
	  //std::cout << "AlprImpl::recognize " << diffclock(startTime, endTime) << "ms." << std::endl;
  ...
  std::queue<PlateRegion> AlprImpl::recognizeregion( unsigned char* pixelData, int bytesPerPixel, int imgWidth, int imgHeight, std::vector<AlprRegionOfInterest> regionsOfInterest )
  {
      //cout << "teddyxu: recognizeregion" <<endl;
      std::queue<PlateRegion> plateQueue;
      try
      {
          //timespec startTime;
          ...
          return this->recognizeregion(img, this->convertRects(regionsOfInterest));
      }
      catch (cv::Exception& e)
      {
          std::cerr << "Caught exception in OpenALPR recognize: " << e.msg << std::endl;
  ...
  /* teddyxu: add the same function, with ocr matrix included*/
  AlprResults AlprImpl::recognize( unsigned char* pixelData, int bytesPerPixel, int imgWidth, int imgHeight, cv::Mat frm_ocr, std::vector<AlprRegionOfInterest> regionsOfInterest)
  {
    ...
    try
    {
      ...
      int arraySizeOcr = frm_ocr.cols * frm_ocr.rows * bytesPerPixel;
      cv::Mat imgDataOcr = cv::Mat(arraySizeOcr, 1, CV_8U, frm_ocr.data);
      cv::Mat imgOcr = imgDataOcr.reshape(bytesPerPixel, frm_ocr.rows);
      ...
      return this->recognize(img, imgOcr, this->convertRects(regionsOfInterest));
    }
    catch (cv::Exception& e)
    {
      std::cerr << "Caught exception in OpenALPR recognize: " << e.msg << std::endl;
  ...
  std::queue<PlateRegion> AlprImpl::recognizeregion(cv::Mat img, std::vector<cv::Rect> regionsOfInterest)
  {
      timespec startTime;
      timespec endTime;
      getTimeMonotonic(&startTime);

      std::queue<PlateRegion> platequeue = recognizeFullDetailsRegion(img, regionsOfInterest);
    ...
    AlprResults AlprImpl::recognizeresult(cv::Mat colorImg, cv::Mat grayImg, std::queue<PlateRegion> plateQueue)
    {
        timespec startTime;
        timespec endTime;
        getTimeMonotonic(&startTime);

        AlprFullDetails fullDetails = analyzeSingleCountryResult(colorImg, grayImg, plateQueue);
  ...
  /* teddyxu: add the same function, with ocr matrix included*/
  AlprResults AlprImpl::recognize(cv::Mat img, cv::Mat imgOcr, std::vector<cv::Rect> regionsOfInterest)
  {
	timespec startTime;
	timespec endTime;
	getTimeMonotonic(&startTime);

    AlprFullDetails fullDetails = recognizeFullDetails(img, imgOcr, regionsOfInterest);

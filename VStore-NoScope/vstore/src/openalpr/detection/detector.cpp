  ...
  vector<PlateRegion> Detector::detect(Mat frame, std::vector<cv::Rect> regionsOfInterest)
  {
    ...
    for (int i = 0; i < regionsOfInterest.size(); i++)
    {
      ...
      Size maxPlateSize(maxWidth, maxHeight);
        ...
        //cout << "Max " <<  maxWidth << " " << maxHeight << endl;

      //teddyxu: !!!!!!!!!!!!!!!!!!!!!This is the real place that the program detect the plate region!!!!!!!!!!!!!!!!!!!!!
        vector<Rect> allRegions = find_plates(cropped, minPlateSize, maxPlateSize);
        //Rect rect;
        //vector<Rect> allRegions;
      ...
      for( unsigned int i = 0; i < allRegions.size(); i++ )
      {
        ...
        //cout << "here" << endl;
        //cout << allRegions[i].x << " " << allRegions[i].y  << " " << allRegions[i].width << " " << allRegions[i].height << endl;
        //cout << "I am here" << endl;
      }

        //teddyxu: Deceit the program to have the whole picture as the plate
        /*
        for( unsigned int i = 0; i < allRegions.size(); i++ )
        {
            allRegions[0].x = 0;
            allRegions[0].y = 0;
            allRegions[0].width = 62;
            allRegions[0].height = 33;

            // Ensure that the rectangle isn't < 0 or > maxWidth/Height
            allRegions[0] = expandRect(allRegions[0], 0, 0, w, h);

            allRegions[0].x = allRegions[0].x + offset_x;
            allRegions[0].y = allRegions[0].y + offset_y;
            cout << allRegions[0].x << " " << allRegions[0].y  << " " << allRegions[0].width << " " << allRegions[0].height << endl;
        }
        */
...
//      cout << "here" << endl;
  ...
  float Detector::computeScaleFactor(int width, int height) {
    ...
    else if (height > config->maxDetectionInputHeight)
    {
      ...
      scale_factor = ((float) config->maxDetectionIxnputHeight) / ((float) height);

...
//	xzl_bug_on(rc != 0);
    ...
    for (int i = 0; i < THREAD_NUM; i++){
        rc = mdb_env_create(&env_multi[i]);
        xzl_bug_on(rc != 0);

        rc = mdb_env_set_mapsize(env_multi[i], 1UL * 1024UL * 1024UL * 32UL); /* 32 MiB */
        xzl_bug_on(rc != 0);

        rc = mdb_env_open(env_multi[i], db_address[21].c_str(), 0, 0664);
    }
    ...
    while(1){
        ...
        switch (s->rq.db_seq) {
            ...
            case SSD_CHUNK_DIST:
                ...
                //dbpath = db_path + temp[rd->temp].c_str();
                EE("start = %d, end = %d", 2 * rd->start_fnum + 2, 2 * rd->start_fnum + 2 * rd->total_fnum);
                test_send_chunk_from_db(env_multi[rd->id], db_path.c_str(), sender, TYPE_CHUNK, 2 * int(rd->start_fnum/CHUNK_LENGTH) + 2, 2 * int(rd->start_fnum/CHUNK_LENGTH) + 3, rd->id);
                break;
            case SSD_RAW_YOLO:

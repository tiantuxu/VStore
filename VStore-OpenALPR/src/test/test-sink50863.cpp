    ...
    while(1){
        ...
        for (int j = rd.start_fnum; j < rd.start_fnum + rd.total_fnum; j++) {
            ...
            if (regionsOfInterest.size() > 0){
                ...
                //Specify the frame wants to retrieve
                rd_one_frame_ocr.db_seq = HDD_RAW_540;
                ...
                if(plateQueue.size() > 0) {
                    ...
                    if(j-status_540[id]<=CHUNK_SIZE)
                        flag_540[id]=false;                                    // ///*premise one kind of request at a time?
                    ...
                    //*************************************************//
                    if(j - status_540[id] == 4*CHUNK_SIZE||flag_540[id]==true) {      // ///*premise one kind of request at a time?**/////////////////////////////////????
                        flag_540[id]=true;
                        rd_one_frame_ocr.db_seq = HDD_RAW_540;

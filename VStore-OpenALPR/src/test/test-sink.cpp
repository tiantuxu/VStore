...
//	receiver.connect(CLIENT_PULL_FROM_ADDR);
    ...
    //while(1){
        ...
        #pragma omp parallel for schedule(dynamic, 250) num_threads(40)
        for (int j = 0; j < rd.total_fnum; j++) {
            ...
            if (regionsOfInterest.size() > 0){
                ...
                if (rd_one_frame.db_seq == SSD_CHUNK_DIST) {
                    ...
                    if (j - status_en_720[id] >= CHUNK_LENGTH) {
                        ...
                        for (int iter = 0; iter < rd_one_frame.total_fnum; iter++) {
                            ...
                            //msg_ptr2 = recv_one_chunk_720(*worker_chunk_socket[id]);
                ...
                queue<PlateRegion> plateQueue = recognizeRegion(alpr[id], frm_720, "", outputJson, id, j, regionsOfInterest, 1);
			...
			cvtColor(frm_720, frm_ocr_gray, CV_BGR2GRAY);
                    }

                    getTimeMonotonic(&startTime);
                    //bool plate_found = recognizePlate(alpr[id], frm_ocr, frm_ocr_gray, "", outputJson, id, j, plateQueue);
		    bool plate_found = recognizePlate(alpr[id], frm_720, frm_ocr_gray, "", outputJson, id, j, plateQueue);
                    getTimeMonotonic(&endTime);
                    std::cout << "Total Plate Recognition: " << diffclock(startTime, endTime) << "ms." << std::endl;
